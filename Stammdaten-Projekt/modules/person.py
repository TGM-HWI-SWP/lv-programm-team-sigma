import datetime as dt # import datetime as datetime
...
class person:
    ...
    table_name = "PERSON"
    table_row_names = [
    ["PERS_ID", "integer PRIMARY KEY", "Personen-ID"],
    ...
    def select_specific(cls, db_ms : dbms.dbms, id : int) -> List["person"]:
        ...
        """
        obj_list = []
        list_from_db = db_ms.select_specific(table_name=cls.table_name, limitations=f"{cls.table_row_names[0][0]} = {id}")
    ...
    def value(self) -> tuple:
        ...
        """
        return (self.obj_id, self.name, self.surname, self.__birthdate, self.street, self.housenr, self.floor, self.zip, self.place)
