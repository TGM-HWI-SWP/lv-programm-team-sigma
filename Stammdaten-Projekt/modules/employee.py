...
class mitarbeiter(person.person):
    ...
    @classmethod
    def initialize_db_table(cls, dbms : dbms.dbms):
        """
        Erstellt die Datenbanktabelle anhand der statischen Attribute
        Args:
            dbms (dbms.dbms): Die verbundene Datenbanken
        """
        dbms.create_table(table_name=cls.table_name, table_row_name=cls.table_row_names)

    @classmethod
    def select_all(cls, dbms : dbms.dbms) -> List["mitarbeiter"]:
        """
        Lädt alle 
        Args:
            dbms (dbms.dbms): Die verbundene Datenbanken
        ...
        """
        obj_list = []
        list_from_db, description = dbms.select_all(table_name=cls.table_name, join_table_name=person.person.table_name, table_row_FK=cls.table_row_names[1][0],  join_table_PK=person.person.table_row_names[0][0])
        #print(list_from_db)
        if list_from_db == None:
            return obj_list
            print(f"Keine Einträge geladen!")
        for (id, persid, eintrittsdatum, gehalt, persid1, surname, firstname, birthdate, street, housenr, floor, zip, place) in list_from_db:
            try:
                ...
                ... #print(obj_list)
            # print(f"{len(list_from_db)} Einträge geladen!")
        return obj_list
    
    @classmethod
    def select_specific(cls, dbms : dbms.dbms, id : int) -> List["mitarbeiter"]:
        """
        Lädt alle 
        Args:
            dbms (dbms.dbms): Die verbundene Datenbanken
        ...
        """
        obj_list = []
        list_from_db, description = dbms.select_specific(table_name=cls.table_name, join_table_name=person.person.table_name, table_row_FK=cls.table_row_names[1][0],  join_table_PK=person.person.table_row_names[0][0], limitations=f"{cls.table_row_names[0][0]} = {id}")
        if list_from_db == None:
            return obj_list
        for (id, persid, eintrittsdatum, gehalt, persid1, surname, firstname, birthdate, street, housenr, floor, zip, place) in list_from_db:
            try:
    ...
    def update(self, dbms : dbms.dbms, values : tuple):
        """
        Updated die Daten des Mitarbeiters in der Datenbank
        Args:
            dbms (dbms.dbms): Die verbundene Datenbanken
        """
        dbms.update(table_name=self.table_name, table_row_name=self.table_row_names[:-1], values=values, primary_key=self.table_row_names[0][0], primary_key_value=self.empolyee_ID)
        
    def delete(self, dbms : dbms.dbms):
        """
        Löscht den Mitarbeiter aus der Datenbank
        Args:
            dbms (dbms.dbms): Die verbundene Datenbanken
        """
        dbms.delete(table_name=self.table_name, table_idrow_name=self.table_row_names[0][0], id=self.empolyee_ID)